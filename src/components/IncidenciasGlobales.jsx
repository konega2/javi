...
function IncidenciasGlobales({ onBack, userName, onLogout }) {
  ...
  // Filtrado y ordenamiento de incidencias (mismo código que en IncidenciasLista)
  ...
  const contarPorZona = () => {
    ...
    incidencias.forEach(inc => {
      conteo[inc.zona] = (conteo[inc.zona] || 0) + 1
    })
    return conteo
  }

  const contarPorRiesgo = () => {
  ...
  const conteoPorZona = contarPorZona()
  const conteoPorRiesgo = contarPorRiesgo()
    ...
    <div>
      ...
      <div className="mb-8">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-3xl font-bold text-gray-800">Todas las Incidencias</h2>
            <p className="text-gray-600 mt-2">{incidenciasFiltradas.length} de {incidencias.length} incidencias del sistema</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div className="bg-white rounded-2xl shadow-md p-6">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Por Nivel de Riesgo</h3>
          <div className="space-y-2">
            {Object.entries(conteoPorRiesgo).map(([riesgo, cantidad]) => (
              <div key={riesgo} className="flex justify-between items-center">
                <span className={`px-3 py-1 rounded-full text-sm font-medium ${obtenerColorRiesgo(riesgo)}`}>
                  {riesgo}
                </span>
        ...
        <div className="bg-white rounded-2xl shadow-md p-6">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Por Zona</h3>
          <div className="space-y-2 max-h-40 overflow-y-auto">
            {Object.entries(conteoPorZona)
              .sort((a, b) => b[1] - a[1])
              .slice(0, 10)
              .map(([zona, cantidad]) => (
                <div key={zona} className="flex justify-between items-center">
                  <span className="text-sm text-gray-600 truncate">{zona}</span>
                  <span className="font-bold text-gray-700 ml-2">{cantidad}</span>
                </div>
              ))}
          </div>
        </div>
      ...
      {/* Filtros (igual que en IncidenciasLista) */}
...
// Reutilizar el componente DetalleIncidencia de IncidenciasLista
...
export default IncidenciasGlobales